...
class CapabilityRegistry:
    ...
    def default_storage_dir(cls):
        ...
        return Path(__file__).resolve().parents[1] / cls.DEFAULT_STORAGE_RELATIVE
    ...
    def _paths(self, capability):
        return self.records_dir / f"{capability.id}.json", self.sources_dir / f"{capability.id}_{self._safe_name(capability.name)}.py"

    @staticmethod
    def _event_to_dict(event):
        return {"event": event.event, "detail": event.detail, "timestamp": event.timestamp}

    def _record_for(self, capability):
        record_path, source_path = self._paths(capability)
        return {"schema_version": self.SCHEMA_VERSION, "id": capability.id, "name": capability.name, "version": capability.version, "state": capability.state, "input_types": list(capability.input_types), "output_type": capability.output_type, "parent_id": capability.parent_id, "children_ids": list(capability.children_ids), "created_at": capability.created_at, "activated_at": capability.activated_at, "status": "active" if capability.state == "S1" else capability.state.lower(), "handler": capability.inspect_handler(), "source_path": str(source_path), "record_path": str(record_path), "source_code": capability.source_code, "events": [self._event_to_dict(event) for event in capability.events]}
    ...
    def _persist_capability(self, capability):
        ...
        record_path.write_text(json.dumps(self._record_for(capability), indent=2, sort_keys=True), encoding="utf-8")

    def save(self):
        if not self.storage_dir:
            return None
        self._ensure_storage()
        index = {"schema_version": self.SCHEMA_VERSION, "capabilities": [{"id": c.id, "name": c.name, "state": c.state, "parent_id": c.parent_id, "children_ids": list(c.children_ids), "record_path": str(self._paths(c)[0]), "source_path": str(self._paths(c)[1])} for c in sorted(self._caps.values(), key=lambda item: (item.created_at, item.id))]}
        path = self.storage_dir / "registry.json"
    ...
    def load(self):
        ...
        for item in index.get("capabilities", []):
            ...
            capability = Capability(id=data["id"], name=data["name"], version=data["version"], state=data["state"], input_types=list(data["input_types"]), output_type=data["output_type"], source_code=data["source_code"], parent_id=data.get("parent_id"), children_ids=list(data.get("children_ids", [])), created_at=data["created_at"], activated_at=data.get("activated_at"), events=[Event(e["event"], e.get("detail", ""), e["timestamp"]) for e in data.get("events", [])])
    ...
    def list_active(self):
        return [c for c in sorted(self._caps.values(), key=lambda item: (item.created_at, item.id)) if c.state == "S1"]
    ...
    def children(self, parent_id):
        ...
        return [self._caps[child_id] for child_id in parent.children_ids if child_id in self._caps]
    ...
    def inspect(self, capability_id_or_name):
        ...
        if self.storage_dir:
            record_path, source_path = self._paths(capability)
            storage = {"registry": str(self.storage_dir / "registry.json"), "record": str(record_path), "source": str(source_path)}
        else:
            storage = {"registry": None, "record": None, "source": None}
        return {"id": capability.id, "name": capability.name, "version": capability.version, "state": capability.state, "parent_id": capability.parent_id, "children_ids": list(capability.children_ids), "created_at": capability.created_at, "activated_at": capability.activated_at, "input_types": list(capability.input_types), "output_type": capability.output_type, "source_code": capability.source_code, "handler": capability.inspect_handler(), "storage": storage, "events": [self._event_to_dict(event) for event in capability.events]}
