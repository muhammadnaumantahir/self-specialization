class CapabilityDispatcher:
    ...
    def _find_executable(self, name, input_types):
        ...
        candidates = [c for c in self.registry.all() if self._operation_alias(c.name) == alias and c.input_types == list(input_types) and self._is_executable(c)]
        return candidates[-1] if candidates else None

    def _find_source_for_specialization(self, name):
        alias = self._operation_alias(name)
        candidates = [c for c in self.registry.all() if self._operation_alias(c.name) == alias and c.state in {"S0", "S1"} and c.name != self.GENERAL_PARENT_NAME]
        return candidates[-1] if candidates else None
    ...
    def execute(self, name, a, b, specialization=None):
        ...
        if result.state != "S1":
            failure_events = [event.detail for event in result.events if event.event in {"VERIFY_FAIL", "FAILED"}]
            reason = failure_events[-1] if failure_events else "no failure detail recorded"
            raise RuntimeError(f"Specialization failed: state={result.state}; reason={reason}")
