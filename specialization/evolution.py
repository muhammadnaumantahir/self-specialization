...
class EvolutionEngine:
    ...
    def _create_general_parent(self, source):
        ...
        if existing is not None:
            return existing
        general = Capability.create("SerializeCapability", "1.0", "S0", ["Any", "Any"], "Any", SERIALIZE_SOURCE)
    ...
    def evolve(self, parent_id, target_name, input_types, output_type, cases, source_capability_id=None):
        ...
        try:
            generated = self.specialization_engine.specialize(child, target_name, input_types, output_type, final_parent_id=general_parent.id)
        except Exception as exc:
            child.state = "FAILED"
            child.record("FAILED", f"SPECIALIZATION_ERROR: {type(exc).__name__}: {exc}")
            return child
        try:
            ...
            generated.state = "FAILED"
            generated.record("FAILED", f"VERIFICATION_ERROR: {type(exc).__name__}: {exc}")
            self.registry.register(generated)
            return generated
    ...
    def specialize_request(self, parent_id, target_name, input_types, output_type, cases, source_capability_id=None):
        ...
        if existing is not None:
            return existing
        return self.evolve(parent_id, target_name, input_types, output_type, cases, source_capability_id=source_capability_id)
